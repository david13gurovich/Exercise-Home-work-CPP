...
WriteRequest::WriteRequest(char *rcvMsg, int sockfd, struct sockaddr_in* cliaddr)
    :   m_sockfd(sockfd), 
        m_cliaddr(cliaddr), 
...
void ReplayWrite::SendWrite(int sockfd, struct sockaddr_in* cliaddr)
{
...
void WriteRequest::Action()
{
    ...
    ReplayWrite toSend(m_rcvFromClient.m_uid, 1);
    
    toSend.SendWrite(m_sockfd, m_cliaddr);
} 
}//ilrd

extern "C"
{
    ilrd::Command* Create(char *rcvMsg, int sockefd, struct sockaddr_in* cliaddr)
    {
        return (new ilrd::WriteRequest(rcvMsg, sockefd, cliaddr));
    }
}
